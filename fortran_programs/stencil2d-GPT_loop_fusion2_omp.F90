...
contains
    ...
    subroutine apply_diffusion(in_field, out_field, alpha, num_iter)
        ...
        ! Allocate a temporary field to hold intermediate results
        allocate(tmp1_field(size(in_field, 1), size(in_field, 2), size(in_field, 3)))

        do iter = 1, num_iter

            ! Update the halo first
            call update_halo(in_field)

            ! Compute the first Laplacian - Parallelize only the outermost loop
            ...
            ! Compute the second Laplacian using the result of the first
            !$omp parallel do private(j, k, i)
            do k = 1, nz
                do j = 1 + num_halo, ny + num_halo
                    do i = 1 + num_halo, nx + num_halo
                        out_field(i, j, k) = in_field(i, j, k) - alpha * (-4._wp * tmp1_field(i, j, k) &
            ...
            ! Copy out_field to in_field if not the last iteration
    ...
    subroutine update_halo(field)
        ...
        ! bottom and top edges (without corners)
        do k = 1, nz
            do i = 1 + num_halo, nx + num_halo
                do j = 1, num_halo
                    field(i, j, k) = field(i, j + ny, k)
                    field(i, j + ny + num_halo, k) = field(i, j + num_halo, k)
                end do
            end do
        end do
        ...
        do k = 1, nz
            do j = 1, ny + 2 * num_halo
                do i = 1, num_halo
                    ...
                    field(i + nx + num_halo, j, k) = field(i + num_halo, j, k)
                end do
            end do
        end do
