...
contains
    ...
    do iter = 1, num_iter
      ...
      !$acc data copyin(in_field(1:nx+2*num_halo, 1:ny+2*num_halo, 1:nz))
      !$acc data copyout(out_field(1:nx+2*num_halo, 1:ny+2*num_halo, 1:nz))
      !$acc data copyout(tmp_field(1:nx+2*num_halo, 1:ny+2*num_halo))
      !$acc parallel loop present(in_field, out_field, tmp_field)
      do k = 1, nz
      ...
      if (iter /= num_iter) then
        !$acc parallel loop present(in_field, out_field)
        do k = 1, nz
          do j = 1 + num_halo, ny + num_halo
            do i = 1 + num_halo, nx + num_halo
