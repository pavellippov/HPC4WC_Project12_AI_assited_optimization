...
contains
    ...
    subroutine apply_diffusion(in_field, out_field, alpha, num_iter)
        ...
        ! Allocate and initialize temporary fields if not already allocated
        ...
        do iter = 1, num_iter
            ...
            ! Parallelized first laplacian computation
            !$omp parallel do collapse(3) private(i, j, k) shared(in_field, tmp1_field)
            do k = 1, nz
            ...
            ! Parallelized second laplacian computation
            !$omp parallel do collapse(3) private(i, j, k) shared(tmp1_field, tmp2_field)
            do k = 1, nz
            do j = 1 + num_halo, ny + num_halo
            do i = 1 + num_halo, nx + num_halo
            ...
            ! Parallelized time-stepping update
            !$omp parallel do collapse(3) private(i, j, k) shared(in_field, out_field, tmp2_field)
            do k = 1, nz
            do j = 1 + num_halo, ny + num_halo
            do i = 1 + num_halo, nx + num_halo
            ...
            if (iter /= num_iter) then
                !$omp parallel do collapse(3) private(i, j, k) shared(in_field, out_field)
                do k = 1, nz
                do j = 1 + num_halo, ny + num_halo
                do i = 1 + num_halo, nx + num_halo
